...
public class StaffActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// Create the json fragment and add it as our sole content.
		if (getFragmentManager().findFragmentById(android.R.id.content) == null) {
			JSONFragment json = new JSONFragment();
			getFragmentManager().beginTransaction().add(android.R.id.content, json).commit();
		}
	}
	
	public static class JSONFragment extends ListFragment implements JSONAsyncTaskCompleteListener {
		private static final String LOG_TAG = "StaffActivity$JSONFragment";
